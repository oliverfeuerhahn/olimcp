...
await client.authentication.generateJwt({
  maxRetries: 5,
});
```
...
const response = await client.authentication.generateJwt().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: response, response: raw } = await client.authentication.generateJwt().withResponse();
...
1. Via the `OLI_EMBEDDED_MCP_LOG` environment variable
...
const client = new OliEmbeddedMcp({
  logger: logger.child({ name: 'OliEmbeddedMcp' }),
...
const client = new OliEmbeddedMcp({ fetch });
```
...
```ts
import OliEmbeddedMcp from 'npm:oli-embedded-mcp';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/oli-embedded-mcp-typescript/issues) with questions, bugs, or suggestions.
